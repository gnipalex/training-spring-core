...
public class DefaultDiscountService implements DiscountService {
    ...
    public double getDiscount(User user, Event event,
        ...
        List<Double> discounts = evaluateDiscounts(user, event, airDateTime,
                numberOfTickets);
    ...
    private List<Double> evaluateDiscounts(User user, Event event,
            LocalDateTime airDateTime, long numberOfTickets) {
        return discountStrategies
                .stream()
                .map(strategy -> strategy.getDiscount(user, event, airDateTime,
                        numberOfTickets)).collect(Collectors.toList());
    }
    ...
    public double getDiscount(OrderEntry orderEntry) {
        ...
        for (Entry<Long, Map<LocalDateTime, List<Ticket>>> eventEntry : ticketsGroupedByEventIdAndDate
                .entrySet()) {
            Event event = eventDao.getById(eventEntry.getKey());

            Map<LocalDateTime, List<Ticket>> ticketsGroupedByDate = eventEntry
                    .getValue();

            for (Entry<LocalDateTime, List<Ticket>> dateEntry : ticketsGroupedByDate
                    .entrySet()) {
                LocalDateTime dateTime = dateEntry.getKey();
                long ticketsCount = dateEntry.getValue().size();

                totalOrderEntryDiscount += getDiscount(user, event, dateTime,
                        ticketsCount);
            }
        }
    ...
    private Map<Long, Map<LocalDateTime, List<Ticket>>> getTicketsGroupedByEventAndAirDate(
    ...
    private Map<Long, Map<LocalDateTime, List<Ticket>>> groupByEventAndDate(
            Set<Ticket> tickets) {
        return tickets.stream().collect(
                Collectors.groupingBy(Ticket::getEventId,
